...
MainWindow::auto_filter(cv::Mat magnitude_image)
{
    ...
    cv::circle( mask, cv::Point( mask.cols/2, mask.rows/2 ), 30, cv::Scalar(0), cv::FILLED );
...
//                                                : auto_filter( magnitude_image );
       ...
       //cv::Mat freq_filter_mask = manual_filter( magnitude_image );
       ...
       cv::imshow(" Filter Image", normal_real_plane );
}
